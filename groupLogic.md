...
• Ordering within Strongest: If multiple Strongest groups exist, they are internally ranked by (1) how precisely they match project capacity, (2) the number of distinct majors, and (3) the group’s average CWA (higher averages ranked higher).
...
• Ordering within Strong: Groups are sorted by major diversity first, then by average CWA.
...
• Ordering within Medium: Groups are sorted by average project rank (lower is better), then by CWA balance, then by major diversity.
...
• Remaining students who are not part of any other suggestion are grouped together based on a mix of major diversity and CWA similarity.
...
• Ordering within Weak: Groups are sorted by mutual-like density (ratio of mutual likes to total possible connections), then by average CWA, then by major diversity.
...
• Ranking order: When displayed to administrators, suggested groups are ordered first by strength category (Strongest → Strong → Medium → Weak), and within each category by the group’s average CWA.
